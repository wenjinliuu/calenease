...
final class ScheduleStore {
    ...
    nonisolated static let fileName = "calenease.json"
    /// 改名前（循环班表）的数据文件名。
    nonisolated static let legacyFileName = "shift-ledger.json"

    /// 新文件还没有、旧文件在：把旧文件原样搬成新名字。只搬一次，搬不动就原地读旧文件，
    /// 数据一条都不会丢。备份、网页版导出的 JSON 结构没变，照样能导入。
    nonisolated static func migrateLegacyFile(to url: URL) -> URL {
